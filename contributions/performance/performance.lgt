:- object(performance,
	...
	init :-
		...
		event_registry::set_monitor(_, solve(_, _, _, _, _), _, Self),
		after_event_registry::set_monitor(_, next_state(_, _, _, _), _, Self),
		event_registry::set_monitor(_, solve(_, _, _, _, _, _), _, Self),
	...
	after(_, next_state(_, S1, S2, _), _) :-
		!,
		(	retract(transitions_(S1, S2, N)) ->
			N2 is N + 1
		;	N2 is 1
	...
	after(_, solve(_, _, _, _, Solution), _) :-
		!,
		length(Solution, Length),
		retractall(solution_length_(_)),
		asserta(solution_length_(Length)).
	after(_, solve(_, _, _, _, Solution, _), _) :-
		!,
		length(Solution, Length),
		retractall(solution_length_(_)),
		asserta(solution_length_(Length)).
