...
:- initialization((
	...
	write(user_output, ' - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -'), 
	nl(user_output),
	write(user_output, '	Loading process model: Dependency Test Model                              '), 
	nl(user_output),
	write(user_output, ' - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -'), 
	nl(user_output),
	write(user_output, '+++ Transforming ontologies used within process model...                      '),
	logtalk::expand_library_path(be_dep_test('organization.owl'), Path1),
	owl2_to_logtalk:load_axioms(Path1),
	logtalk::expand_library_path(be_dep_test('tools.owl'), Path2),
	owl2_to_logtalk:load_axioms(Path2),
	logtalk::expand_library_path(be_dep_test('popm_objects.lgt'), Path3),
	owl2_to_logtalk:save_axioms_ontology(Path3, owlpl, [no_base(_)]),
	logtalk::expand_library_path(be_dep_test('popm_individuals.lgt'), Path4),
